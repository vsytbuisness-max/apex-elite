...
async function getInsightsData() {
  try {
    ...
    const pageSettings = await client.fetch(`*[_type == "insightsPage"][0]`);
    ...
    return { pageSettings: null, posts: null };
  }
}
...
export default async function Insights() {
  const { pageSettings, posts: cmsPosts } = await getInsightsData();
  const activePosts = cmsPosts?.length ? cmsPosts : defaultBlogs;
      ...
      <div className="max-w-7xl mx-auto px-6 mb-16 text-center">
        <div className="animate-in fade-in slide-in-from-bottom-8 duration-1000">
          <h1 className="font-heading text-5xl md:text-7xl font-bold mb-6 tracking-tight">
            {pageSettings?.title || "Industry Insights."}
          </h1>
          <p className="text-secondary max-w-2xl mx-auto text-lg">
            {pageSettings?.subtitle || "Expert perspectives on modern architecture, luxury home trends, and the future of residential construction."}
          </p>
        </div>
      </div>
